...
const validateItem = [
  body("description").isString().isLength({min: 10, max: 300}).withMessage("IC001:Description lenght must lay between 10 and 300 characters"),
  body("stock").isNumeric().custom(isNotNeg),
  body("name").isString().isLength({min: 0}),
  body("image").isString().isLength({min: 0}).withMessage("BR004:Field image cannot be of length 0"),
